...
const DepotList_In_SearchUrl = $store.state.mock.url.DepotList_In_SearchUrl;

const DepotList_In_Search = {
    'success': true,
    'Depotdata': [
        {
            "data": {
                ...
                'supplierarticleNum': /^[A-Z0-9]{5}[0-9]{5}[A-Z]{5}$/,                        // 供应商货号
                'accessories|1': [                                                            // 面辅料名称
                    '羊绒', '羊毛', '兔毛', '羊驼毛', '人丝', '真丝', '黄麻', '亚麻', '大麻', '腈纶', '氨纶',
                    '莱卡', '尼龙', '金属丝', '涂层', '竹纤维', '莫代尔', '桑蚕丝', '人棉', '榨蚕丝', '马海毛'
                ],
                ...
                'remarks': function (n) {                                                                    // 备注
                    return n.context.currentContext.handledate + " 入库，面辅料名称：" +
...
/**
 * mockTest_DepotList_In_Search
 * @param url
 * @param tem
 */
const mockTest_DepotList_In_Search = function (url, tem) {
    Mock.mock(url, tem);
};

export {DepotList_In_SearchUrl, DepotList_In_Search, mockTest_DepotList_In_Search}
