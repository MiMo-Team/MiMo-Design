<template>
    <div class="wm" id="announcement">
        <!--tit-->
        <p class="win-tit"><span>▌</span>&nbsp;{{title}}</p>
        <!--删选/操作 模块-->
        <sticky className="sticky-navbar" :stickyTop="50" :stickyHeight="56">
            <el-row :span="24" class="filter-container">
                <el-row :gutter="10">
                    ...
                    <!--刷新-->
                    <el-col :xs="24" :sm="8" :md="6" :lg="2">
                        <el-button class="filter-item-btn" type="primary" v-waves icon="" @click.native="refreshList">
                ...
                <template scope="scope">
                    ...
                    <el-dropdown style="margin-left: 9px">
                        ...
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item @click.native.prevent="handleModifyStatus(scope.row, 'published')">发布</el-dropdown-item>
                            <el-dropdown-item @click.native.prevent="handleModifyStatus(scope.row, 'draft')">草稿</el-dropdown-item>
                            <el-dropdown-item @click.native.prevent="handleModifyStatus(scope.row, 'off')">隐藏</el-dropdown-item>
                        </el-dropdown-menu>
                    </el-dropdown>
                </template>
            </el-table-column>
        ...
        <!--弹出编辑框-->
        <dialog-announcement :temp="temp" :typeOptions="typeOptions" v-on:tempChange="tempChange" v-if="temp.dialogFormVisible"></dialog-announcement>
    </div>
</template>
<script>
    import dialogAnnouncement from '../../components/dialog/dialog-announcement.vue';
    ...
    export default {
        ...
        created(){
            /*无删选状态加载所有数据*/
            this.getList(this, this.$store.state.mock.url.announcementUrl, {}, this.Filter.listpage.currentPage, this.Filter.listpage.pageSize, (r) => {
                this.storageData = r.announcement;
            }, () => {
        ...
        methods: {
            ...
            handleDownload(){
                require.ensure([], () => {
                    ...
                    export_json_to_excel(tHeader, data, 'MiMo服装设计后台管理系统 - 内部通告数据');
                })
            },
            //excel 转换显示形式
